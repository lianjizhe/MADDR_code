...
class ProgGrowImageGenerator(AbstractProgGrowGenerator):
    def __init__(self, dataset, max_resolution, batch_size, inf=False):
        ...
        self._img_label_gen = iter(self._data_loader)

        # debug
        # for i in self._img_label_gen:
        #     print(i)
    ...
    def __next__(self):
        # if self._img_label_gen is None:
        #     print("None None None") #debug 此处并没有打印
        # self._img_label_gen = iter(self._data_loader)
        images_labels_list = next(self._img_label_gen, None)
        if images_labels_list is None:
            if self._inf:
                # del self._image_gen
                self._img_label_gen = iter(self._data_loader)
                images_labels_list = next(self._img_label_gen, None)
                images = images_labels_list[0]
                labels = images_labels_list[1]
            else:
                raise StopIteration()
        else:
            labels = images_labels_list[1]
        ...
        return self._mix_res_module(images).cuda(),labels.cuda()
