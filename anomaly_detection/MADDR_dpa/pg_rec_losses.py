...
class PGPerceptualLoss(AbstractPGLoss):
    ...
    def forward(self, x, fake_z, dec, label, zs, zc_logit):
        # print("hello PGPerceptualLoss")
        self._loss.set_new_weights(**self._weights_per_resolution[self._resolution])
        loss = self._loss(x, fake_z, dec, label, zs, zc_logit)
        ...
        #         prev_loss = self._loss(x, y, fake_z, dec, label, zs, zc_logit)
...
class PGL1Loss(AbstractPGLoss):
    ...
    def forward(self, x, fake_z, dec, label, zs, zc_logit):
        return self._loss(x, fake_z, dec, label, zs, zc_logit)
...
class PGComposeLoss(AbstractPGLoss):
    ...
    def forward(self, x, fake_z, dec, label, zs, zc_logit):
        # print("hello PGComposeLoss")
        # loss_2算的是图片之间的mse，在这边不好给 
        return self.loss_1_weight * self.loss_1(x, fake_z, dec, label, zs, zc_logit) + self.loss_2_weight * self.loss_2(x, fake_z, dec, label, zs, zc_logit)
